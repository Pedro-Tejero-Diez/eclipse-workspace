...
public class POOEjercicio834 {
	...
	public static void main(String[] args) {
		...
		Hotel hotel2 = new Hotel("Manolita", 3, 535, 12);
	...
	private static int menu() {
		...
		do {
			...
			System.out.println("2. Opció 2. Dar de Baja Edificio");
			System.out.println("3. Opció 3. Mostrar Edificio");
			System.out.println("4. Opció 4. Mostrar Todos los edificios");
	...
	public static void mostrarEdificio(ArrayList<Edificio> listado) {
		...
		if (buscarEdificio(listado, name) != null) {
			...
			System.out.println(edificio.limpieza());
			System.out.println(edificio.calcularCosteVigilancia());

			if (edificio instanceof Cine) {
				// aqui utilizo mi sistema y no casteo cine, por probar
				Cine cine = (Cine) edificio;
				System.out.println("introduzca precio entrada:");
				int precioEntrada = sc.nextInt();
				System.out.println("introduzca numero asistentes:");
				int numeroAsistentes = sc.nextInt();
				System.out.println(cine.proyectarSesion(precioEntrada, numeroAsistentes));

			} else if (edificio instanceof Hotel) {
				// Hotel hotel = (Hotel) edificio; hay otro sistema que ahorra lineas: castear
				// al tipo
				System.out.println(((Hotel) edificio).calculoPersonas());

			} else {
				// Hospital hospital = (Hospital) edificio; mi sistema. mejor castear, ahorras
				// lineas codigo
				System.out.println(((Hospital) edificio).repartirComida());
	...
	public static void mostrarEdificios(ArrayList<Edificio> listado) {

		for (Edificio edificio : listado) {
			System.out.println("Tipo de edificio = "+edificio.getClass()+"\n NOMBRE: "+edificio.getNombre());
			System.out.println(edificio.limpieza());
			System.out.println(edificio.calcularCosteVigilancia());
			...
			if (edificio instanceof Hotel) {
				...
				System.out.println(((Cine) edificio).proyectarSesion(25,15));
			System.out.println();
			System.out.println();
		}
